...
pub struct FileData {
    ...
    /// For PDF files: empty (rendered on demand by the native PDF renderer).
...
pub(crate) fn render_pdf_pages(
    ...
    log::info!(
        "WinRT PDF rendering: {} pages, dpi={}, jpeg={}",
        page_count,
        dpi,
        use_jpeg
    );
    ...
    for i in 0..page_count {
        ...
        let options =
            PdfPageRenderOptions::new().map_err(|e| format!("创建渲染选项失败: {}", e))?;
        options
            .SetDestinationWidth(dest_w)
            .map_err(|e| format!("设置宽度失败: {}", e))?;
        options
            .SetDestinationHeight(dest_h)
            .map_err(|e| format!("设置高度失败: {}", e))?;
        ...
        let stream_size = stream
            .Size()
            .map_err(|e| format!("获取流大小失败: {}", e))? as u32;
        ...
        let (data_url, format) = if use_jpeg {
            let img =
                image::load_from_memory(&png_data).map_err(|e| format!("解码PNG失败: {}", e))?;
...
#[cfg(target_os = "windows")]
pub(crate) fn render_pdf_pages_pdfium(
    pdf_path: &str,
    dpi: u32,
    use_jpeg: bool,
) -> Result<Vec<RenderedPage>, String> {
    ...
    let pdf_bytes = std::fs::read(pdf_path).map_err(|e| format!("读取PDF文件失败: {}", e))?;

    let images = crate::pdfium_print::render_pdf_to_images(&pdf_bytes, dpi)?;

    let results: Vec<RenderedPage> = images
        .into_iter()
        .map(|img| {
...
pub fn read_invoice_files(paths: Vec<String>) -> Result<Vec<FileData>, String> {
    ...
    let valid_paths: Vec<(String, String, String, u64)> = paths
        .iter()
        .filter_map(|path_str| {
            ...
            if ![
                "pdf", "jpg", "jpeg", "png", "bmp", "webp", "tiff", "tif", "ofd", "xml",
            ]
            .contains(&ext.as_str())
            {
                return None;
    ...
    // For PDF files, data_url is empty because pages are rendered on demand.
    ...
    let parallel_results: Vec<FileData> = non_ofd_paths
        ...
        .filter_map(|(path_str, name, ext, size)| {
            if ext == "pdf" {
                // PDF files: no data_url needed; the frontend requests rendered pages separately.
            ...
            Some(FileData {
                ...
                path: Some(path_str.clone()),
                orig_w: if img_orig_w > 0 {
                    Some(img_orig_w)
                } else {
                    None
                },
                orig_h: if img_orig_h > 0 {
                    Some(img_orig_h)
                } else {
                    None
                },
...
pub(crate) fn enhance_image(file_path: &str) -> Result<String, String> {
    ...
    let orient = if is_jpeg_bytes(&bytes) {
        read_exif_orientation(&bytes)
    } else {
        1
    };
    let img = if orient != 1 {
        apply_exif_orientation(img, orient)
    } else {
        img
    };
    ...
    unsafe {
        ...
        for i in 0..count_returned {
            ...
            let is_default = default_name
                .as_ref()
                .map_or(false, |dn| dn.eq_ignore_ascii_case(&name));
...
fn find_sumatrapdf_from_registry() -> Option<std::path::PathBuf> {
    ...
    unsafe {
        ...
        let mut typ: windows::Win32::System::Registry::REG_VALUE_TYPE =
            windows::Win32::System::Registry::REG_NONE;
...
/// Structured identically to `invoice_extractor::RecognitionPage` so the frontend can reuse
...
pub fn extract_pdf_text(pdf_path: &str, page_idx: u32) -> Result<PdfTextResult, String> {
    let doc = lopdf::Document::load(pdf_path).map_err(|e| format!("PDF加载失败: {}", e))?;
...
fn extract_pdf_text_from_doc(
    ...
    let page_id = *pages
        .get(&(page_idx + 1)) // lopdf pages are 1-indexed
    ...
    let mut tounicode_cmaps: std::collections::BTreeMap<Vec<u8>, CMap> =
    ...
    let mut lopdf_encodings: std::collections::BTreeMap<Vec<u8>, lopdf::Encoding> =
        std::collections::BTreeMap::new();
    ...
    let mut font_encoding_names: std::collections::BTreeMap<Vec<u8>, Vec<u8>> =
        std::collections::BTreeMap::new();

    // Helper: load ToUnicode CMap from a font dictionary, also record encoding name
    fn load_cmap_from_font(doc: &lopdf::Document, font_dict: &lopdf::Dictionary) -> Option<CMap> {
        ...
        let cmap_bytes = match &resolved_obj {
            ...
            _ => {
                ...
                {
                    flate2::read::ZlibDecoder::new(content_bytes)
                        .bytes()
    ...
    for (name, font_dict) in &page_fonts {
        ...
        if let Ok(enc_name) = font_dict
            .get(b"Encoding")
            .and_then(|e| e.as_name().map(|n| n.to_vec()))
        {
                                                ...
                                                if let Ok(form_res) = stream.dict.get(b"Resources")
                                                {
                                                    let resolved_form_res = match form_res {
                                                        lopdf::Object::Reference(rid) => {
                                                            doc.get_dictionary(*rid).ok()
                                                        }
                                                        _ => form_res.as_dict().ok(),
                                                    };
                                                    if let Some(form_res_dict) = resolved_form_res {
                                                        if let Ok(form_font_dict) =
                                                            form_res_dict.get(b"Font")
                                                        {
                                                            let resolved_fonts =
                                                                match form_font_dict {
                                                                    lopdf::Object::Reference(
                                                                        rid,
                                                                    ) => doc
                                                                        .get_dictionary(*rid)
                                                                        .ok(),
    ...
    _debug_log.push_str(&format!(
        "LopdfEncodings: {}, CMaps: {}, FormXObjs: {}\n",
        lopdf_encodings.len(),
        tounicode_cmaps.len(),
    ...
    log::info!(
        "PDF文本提取: {} lopdf encodings, {} ToUnicode CMaps, {} Form XObjects",
        lopdf_encodings.len(),
        tounicode_cmaps.len(),
        form_xobject_ids.len()
    );
    ...
    let mut xobj_name_to_id: std::collections::HashMap<Vec<u8>, lopdf::ObjectId> =
    ...
    let mut form_cm_params: std::collections::HashMap<Vec<u8>, [f64; 6]> =
        std::collections::HashMap::new();
    {
                ...
                if let Some(name_obj) = ops[i].operands.first() {
                    if let lopdf::Object::Name(name) = name_obj {
                        ...
                        if xobj_name_to_id.contains_key(name) {
                            ...
                            let j = if i > 0 && ops[i - 1].operator == "cm" {
                                i - 1
                            } else if i > 1
                                && ops[i - 1].operator == "q"
                                && ops[i - 2].operator == "cm"
        ...
        for fxobj_id in &form_xobject_ids {
            if let Ok(obj) = doc.get_object(*fxobj_id) {
                if let Ok(stream) = obj.as_stream() {
                    ...
                    let fxobj_name = xobj_name_to_id
                        .iter()
        ...
        match op.operator.as_str() {
            ...
            "Tj" if in_text_block => {
                ...
                if let Some(obj) = op.operands.first() {
                    if let Some(decoded) = decode_text_object(
                        obj,
                        &lopdf_encodings,
                        &tounicode_cmaps,
                        &current_font,
                        ...
                        if !decoded.is_empty() {
                            ...
                            if need_space_before {
                                full_text_parts.push(" ".to_string());
                            }
                    ...
                    for item in arr {
                        match item {
                            Object::String(bytes, _format) => {
                                if let Some(decoded) = decode_bytes_with_encoding(
                                    bytes,
                                    &lopdf_encodings,
                                    &tounicode_cmaps,
                                    &current_font,
                            ...
                            Object::Integer(kern) => {
                                ...
                                if !text_buf.is_empty() && kern_f < KERN_WORD_BREAK {
                                    let (px, py) = apply_ctm(&ctm, cur_x, cur_y);
                                    let word =
                                        make_word(&text_buf, px, py, font_size, page_h_pt, scale);
                                    all_words.push(word);
                                    if need_space_before {
                                        full_text_parts.push(" ".to_string());
                                    }
    ...
    while i < tokens.len() {
        ...
        if token == "beginbfchar" {
            ...
            while i < tokens.len() && tokens[i] != "endbfchar" {
                ...
                if parts.len() >= 2 {
                    if let (Some(glyph), Some(ch)) =
                        (parse_hex_pair(parts[0]), parse_hex_pair(parts[1]))
                    {
                    ...
                    if let (Some(glyph), Some(ch)) =
                        (parse_hex_pair(glyph_str), parse_hex_pair(unicode_str))
                    {
                        if let Some(unicode_char) = std::char::from_u32(ch as u32) {
                            cmap.add_mapping(glyph, unicode_char);
                        }
            ...
            while i < tokens.len() && tokens[i] != "endbfrange" {
                ...
                if all_parts.len() >= 3 {
                    if let (Some(start), Some(end), Some(unicode_start)) = (
                        parse_hex_pair(all_parts[0]),
                        parse_hex_pair(all_parts[1]),
                        parse_hex_pair(all_parts[2]),
                    ) {
...
fn decode_bytes_with_encoding(
    ...
    if let Some(enc_name) = encoding_names.get(font_name) {
        ...
        let _is_sjis =
            enc_str.contains("RKSJ") || enc_str.contains("90ms") || enc_str.contains("83pv");
        ...
        if is_gbk_euc {
            ...
            if !text.is_empty() && text.chars().any(|c| is_cjk(c) || c.is_alphanumeric()) {
                log::debug!(
                    "GBK decode for font {:?} (encoding {}): {} chars",
            ...
            if !text.is_empty() && text.chars().any(|c| is_cjk(c) || c.is_alphanumeric()) {
                log::debug!(
                    "BIG5 decode for font {:?} (encoding {}): {} chars",
                    String::from_utf8_lossy(font_name),
                    enc_str,
                    text.chars().count()
                );
...
pub fn trim_white_edges_with_bounds(
    ...
    let rgba = img.to_rgba8();
    let cropped =
        image::imageops::crop_imm(&rgba, bounds.left, bounds.top, bounds.width, bounds.height);
...
fn calculate_layout_mm(settings: &RenderSettings) -> (Vec<LayoutSlotMm>, f32, f32) {
    ...
    let has_footer = settings.page_num
        || settings.print_date
        || settings
            .footer_text
            .as_ref()
            .map_or(false, |t| !t.is_empty());
    let line_count = (if settings.page_num || settings.print_date {
        1
    } else {
        0
    }) + (if settings
...
fn render_text_overlay(
    ...
    let render_text_at = |img: &mut image::RgbaImage,
                          text: &str,
                          x_start: f32,
                          y_baseline: f32,
                          sf: &ab_glyph::PxScaleFont<&ab_glyph::FontArc>| {
        let mut x = x_start;
        for c in text.chars() {
            ...
            if let Some(q) = font.outline_glyph(glyph) {
                ...
                q.draw(|gx, gy, v| {
                    ...
                    if px >= 0 && py >= 0 && (px as u32) < img.width() && (py as u32) < img.height()
                    {
                        ...
                        if alpha > pixel[3] {
                            *pixel =
                                image::Rgba([text_color[0], text_color[1], text_color[2], alpha]);
                        }
                    }
                });
            }
    ...
    if has_line1 {
        ...
        if has_both {
            ...
            render_text_at(
                &mut img,
                print_date_text,
                img_width as f32 - pd_w - margin_x,
                y_offset,
                &scaled_font,
            );
    ...
    match img.write_to(
        ...
        Ok(()) => {
            log::info!(
                "render_text_overlay: generated {}x{} PNG ({} bytes) for '{}' + '{}'",
                img_width,
                img_height,
                png_buf.len(),
                page_num_text,
...
fn render_slot_numbers(
    ...
    log::info!(
        "render_slot_numbers: called with {} slots, start_number={}",
        slot_positions.len(),
        start_number
    );
...
fn render_watermark(
    ...
    log::info!(
        "render_watermark: text='{}', color='{}', opacity={}, font_size={}mm, angle={}",
        watermark_text,
        color,
        opacity,
        font_size_mm,
...
fn decode_images(files: &[FileSpec], settings: &RenderSettings) -> Vec<Option<ImageSource>> {
    use rayon::prelude::*;
    ...
    let decoded: Vec<Option<ImageSource>> = files
        ...
        .map(|file_spec| {
            ...
            if has_exif_rotation {
                log::info!(
                    "Applying EXIF orientation {} (decoded {}x{})",
                    exif_orientation,
                    img.width(),
                    img.height()
                );
...
fn get_cached_xobj(
    ...
    let (iw_mm, ih_mm, xobj_id) = match source {
        ImageSource::Decoded(img) => {
            ...
            let raw_image = match printpdf::RawImage::from_dynamic_image(rotated) {
                ...
                Err(e) => {
                    log::warn!(
                        "RawImage conversion failed for file {} rot {}: {}",
                        file_idx,
                        rotation,
                        e
                    );
        ...
        ImageSource::JpegPassthrough {
            ...
            if rot == 90 || rot == 270 {
                ...
                let img = match image::load_from_memory(raw_bytes) {
                    ...
                    Err(e) => {
                        log::warn!(
                            "JPEG passthrough fallback decode failed for file {}: {}",
                            file_idx,
                            e
                        );
                        return None;
                ...
                dict.insert(
                    "Type".to_string(),
                    printpdf::xobject::DictItem::Name(b"XObject".to_vec()),
                );
                dict.insert(
                    "Subtype".to_string(),
                    printpdf::xobject::DictItem::Name(b"Image".to_vec()),
                );
                dict.insert(
                    "Width".to_string(),
                    printpdf::xobject::DictItem::Int(*width as i64),
                );
                dict.insert(
                    "Height".to_string(),
                    printpdf::xobject::DictItem::Int(*height as i64),
                );
                dict.insert(
                    "BitsPerComponent".to_string(),
                    printpdf::xobject::DictItem::Int(8),
                );
                dict.insert(
                    "ColorSpace".to_string(),
                    printpdf::xobject::DictItem::Name(color_space.to_vec()),
                );
                dict.insert(
                    "Filter".to_string(),
                    printpdf::xobject::DictItem::Name(b"DCTDecode".to_vec()),
                );
    ...
    let cached = CachedXobj {
        iw_mm,
        ih_mm,
        xobj_id: xobj_id.clone(),
    };
...
fn build_page_ops(
    ...
    for (slot_idx, slot_spec) in page_spec.slots.iter().enumerate() {
        ...
        if slot_idx < slot_positions.len() {
            ...
            log::info!(
                "printpdf fallback: slot[{}] x={:.2}mm y={:.2}mm w={:.2}mm h={:.2}mm",
                slot_idx,
                sp.x_mm,
                sp.y_mm,
                sp.w_mm,
        ...
        let rotate_op = {
            ...
            if rot == 180 {
                ...
                Some(printpdf::XObjectRotation {
                    ...
                    rotation_center_x: printpdf::units::Px(
                        (iw_mm * RENDER_DPI as f32 / 25.4 / 2.0) as usize,
                    ),
                    rotation_center_y: printpdf::units::Px(
                        (ih_mm * RENDER_DPI as f32 / 25.4 / 2.0) as usize,
                    ),
                })
            } else {
                None
...
pub fn generate_pdf_from_layout(
    ...
    let needs_text = request.settings.page_num
        ...
        || (request.settings.watermark
            && request
                ...
                .map_or(false, |t| !t.is_empty()));
    let font_warning =
        if needs_text && !std::path::Path::new("C:\\Windows\\Fonts\\simhei.ttf").exists() {
    ...
    let mut xobj_cache: std::collections::HashMap<(usize, i32), CachedXobj> =
        std::collections::HashMap::new();

    // Pre-load CJK font for text overlay
    let text_font = if request.settings.page_num
        || request.settings.print_date
        || request
            .settings
            .footer_text
    ...
    for (i, page_spec) in request.pages.iter().enumerate() {
        ...
        if let Some((png_bytes, _img_w, _img_h)) = render_text_overlay(
            ...
            match printpdf::RawImage::decode_from_bytes(&png_bytes, &mut warnings) {
                ...
                Err(e) => {
                    log::warn!(
                        "printpdf fallback: page {} text overlay RawImage decode failed: {}",
                        i,
                        e
                    );
                }
            ...
            log::warn!(
                "printpdf fallback: page {} render_text_overlay returned None",
                i
            );
        }
        ...
        let page = printpdf::PdfPage::new(printpdf::Mm(pw), printpdf::Mm(ph), ops);
        doc.pages.push(page);
    ...
    std::fs::write(output_path, &pdf_bytes).map_err(|e| format!("写入文件失败: {}", e))?;
...
fn get_page_effective_box(
    ...
    loop {
        ...
        let dict = match source.get_object(current_id) {
            ...
            Ok(lopdf::Object::Reference(id)) => match source.get_object(*id) {
                Ok(lopdf::Object::Dictionary(d)) => d,
        ...
        let cropbox = dict.get(b"CropBox").or_else(|_| dict.get(b"cropbox")).ok();
...
fn parse_box_array(
    ...
    match box_obj {
        ...
        lopdf::Object::Reference(id) => match source.get_object(*id) {
...
fn remap_references(
    ...
    match obj {
        ...
        Object::Stream(stream) => {
            let dict: lopdf::Dictionary = stream
                .dict
                .into_iter()
                .map(|(k, v)| (k, remap_references(v, source, dest, id_map)))
                .collect();
            ...
            Object::Stream(
                lopdf::Stream::new(dict, stream.content)
                            ...
                            lopdf::Object::Reference(id) => {
                                match doc.get_object(id) {
                                    ...
                                    _ => {
                                        ...
                                        merged
                                            .set(key.clone(), lopdf::Object::Dictionary(sub_dict));
                                        continue;
                                    }
                                }
                            }
...
fn extract_page_as_form_xobject(
    ...
    match rot {
        90 => {
            ...
            prefix.extend_from_slice(format!("0 1 -1 0 {:.4} 0 cm\n", page_w_pt).as_bytes());
        }
        180 => {
        ...
        270 => {
            ...
            prefix.extend_from_slice(format!("0 -1 1 0 0 {:.4} cm\n", page_h_pt).as_bytes());
        }
    ...
    if box_x1.abs() > 0.01 || box_y1.abs() > 0.01 {
        prefix.extend_from_slice(format!("1 0 0 1 {:.4} {:.4} cm\n", -box_x1, -box_y1).as_bytes());
    }
    ...
    let (resources_opt, ref_ids) = source
        ...
        if let Ok(annots_obj) = page_dict.get(b"Annots") {
            ...
            for (annot_idx, annot_id) in annot_refs.iter().enumerate() {
                ...
                let normal_ap_obj = match annot_dict.get(b"AP") {
                    Ok(lopdf::Object::Dictionary(ap_dict)) => match ap_dict.get(b"N") {
                        Ok(obj) => obj.clone(),
                        Err(_) => continue,
                    },
                    Ok(lopdf::Object::Reference(id)) => match source.get_dictionary(*id) {
                        Ok(ap_dict) => match ap_dict.get(b"N") {
                            Ok(obj) => obj.clone(),
                            Err(_) => continue,
                        },
                        Err(_) => continue,
                ...
                let ap_xobj_id = match normal_ap_obj {
                    ...
                    lopdf::Object::Stream(_) => {
                        let remapped =
                            remap_references(normal_ap_obj, source, &mut output_doc, id_map);
                ...
                annot_draw_cmds.extend_from_slice(
                    format!(
                        "q {:.6} {:.6} {:.6} {:.6} {:.6} {:.6} cm /{} Do Q\n",
                        cm_a, cm_b, cm_c, cm_d, cm_e, cm_f, annot_name
                    )
                    .as_bytes(),
                );
            ...
            if !annot_xobjects.is_empty() {
                log::info!(
                    "extract_page_as_form_xobject: processed {} annotation(s)",
                    annot_xobjects.len()
                );
            }
        }
    ...
    if !annot_xobjects.is_empty() {
        if let lopdf::Object::Dictionary(ref mut res_dict) = remapped_resources {
            let xobject_dict = match res_dict.get(b"XObject") {
                Ok(lopdf::Object::Dictionary(d)) => d.clone(),
                Ok(lopdf::Object::Reference(id)) => match output_doc.get_object(*id) {
                    Ok(lopdf::Object::Dictionary(d)) => d.clone(),
            ...
            res_dict.set(
                b"XObject".to_vec(),
                lopdf::Object::Dictionary(merged_xobject),
            );
        }
    }
...
fn build_nup_content_stream(
    ...
    for (adj_idx, (layout_slot_idx, _xobj_id, src_w_pt, src_h_pt)) in form_xobjs.iter().enumerate()
    {
        ...
        log::info!(
            "build_nup: layout_slot[{}] adj[{}] x={:.2}mm y={:.2}mm w={:.2}mm h={:.2}mm",
            layout_slot_idx,
            adj_idx,
            slot.x_mm,
            slot.y_mm,
        ...
        if adj.offset_x != 0.0 {
            offset_x += adj.offset_x * MM_TO_PT;
        }
        if adj.offset_y != 0.0 {
            offset_y -= adj.offset_y * MM_TO_PT;
        } // JS Y+ is down, PDF Y+ is up
        ...
        let matrix: Vec<lopdf::Object> = match rot {
            0 => {
                ...
                vec![
                    lopdf::Object::Real(sx),
                    lopdf::Object::Real(0.0),
                    lopdf::Object::Real(0.0),
                    lopdf::Object::Real(sy),
                    lopdf::Object::Real(offset_x),
                    lopdf::Object::Real(offset_y),
                ]
            }
            90 => {
                ...
                vec![
                    ...
                    lopdf::Object::Real(-sx),
                    lopdf::Object::Real(0.0),
                    lopdf::Object::Real(offset_x + draw_w),
                    lopdf::Object::Real(offset_y),
                ]
            }
            180 => {
                ...
                vec![
                    ...
                    lopdf::Object::Real(-sy),
                    lopdf::Object::Real(offset_x + draw_w),
                    lopdf::Object::Real(offset_y + draw_h),
                ]
            }
            270 => {
...
fn image_to_raw_rgb(img: &image::DynamicImage) -> (Vec<u8>, u8) {
    match img {
        ...
        image::DynamicImage::ImageRgb8(buf) => (buf.as_raw().to_vec(), 3),
        image::DynamicImage::ImageLuma8(buf) => (buf.as_raw().to_vec(), 1),
        other => {
...
fn generate_pdf_passthrough(
    ...
    log::info!(
        "generate_pdf_passthrough: settings.footer_margin={}",
    ...
    let mut source_cache: std::collections::HashMap<String, lopdf::Document> =
        std::collections::HashMap::new();
    // Global ObjectId remapping: source (doc_path, ObjectId) → output ObjectId
    let mut global_id_maps: std::collections::HashMap<
        String,
        std::collections::HashMap<lopdf::ObjectId, lopdf::ObjectId>,
    > = std::collections::HashMap::new();
    ...
    if text_font.is_none() && needs_text_font {
        log::warn!(
            "lopdf hybrid: text overlay enabled but font load failed, overlay will be skipped"
        );
    }
    ...
    for (i, f) in request.files.iter().enumerate() {
        log::info!(
            "lopdf hybrid: file[{}] pdf_path={:?} source_type={:?} data_url_len={} file_path={:?}",
            i,
            f.pdf_path,
            f.source_type,
            f.data_url.len(),
    ...
    for (i, s) in sources.iter().enumerate() {
        match s {
            Some(ImageSource::Decoded(img)) => log::info!(
                "lopdf hybrid: source[{}] Decoded {}x{}",
                i,
                img.width(),
                img.height()
            ),
            Some(ImageSource::JpegPassthrough { width, height, .. }) => log::info!(
                "lopdf hybrid: source[{}] JpegPassthrough {}x{}",
                i,
                width,
                height
            ),
    ...
    for (page_idx, page_spec) in request.pages.iter().enumerate() {
        ...
        for (slot_idx, slot) in page_spec.slots.iter().enumerate() {
            let file_idx = match slot.file_index {
                ...
                _ => {
                    log::info!(
                        "lopdf hybrid: page {} slot {} — no file_index, skip",
                        page_idx,
                        slot_idx
                    );
                    continue;
            ...
            log::info!(
                "lopdf hybrid: page {} slot {} file_idx={} pdf_path={:?} source_type={:?}",
                page_idx,
                slot_idx,
                file_idx,
                file.pdf_path,
            ...
            let (xobj_id, src_w_pt, src_h_pt) = if let Some(pdf_path) = &file.pdf_path {
                ...
                let source_page_id =
                    pages.get(&(page_idx_in_pdf + 1)).copied().ok_or_else(|| {
                        format!("PDF页面{}不存在 (文件: {})", page_idx_in_pdf + 1, pdf_path)
                    })?;
                ...
                let extracted =
                    extract_page_as_form_xobject(source, source_page_id, &mut output_doc, id_map)?;
                ...
                let source = match sources.get(file_idx).and_then(|s| s.as_ref()) {
                    ...
                    None => {
                        log::warn!(
                            "lopdf hybrid: image source[{}] is None, skipping slot",
                            file_idx
                        );
                        continue;
                    }
                ...
                match image_to_lopdf_xobject(source, slot.rotation, &mut output_doc) {
                    ...
                    Err(e) => {
                        log::warn!(
                            "lopdf hybrid: image slot {} encode failed: {}, skipping",
                            file_idx,
                            e
                        );
                        continue;
        ...
        if let Some((png_bytes, _img_w, _img_h)) = render_text_overlay(
            ...
            match image::load_from_memory(&png_bytes) {
                Ok(rgba_img) => {
                    ...
                    let rgb_bytes: Vec<u8> =
                        rgba_img.pixels().flat_map(|p| [p[0], p[1], p[2]]).collect();
                    ...
                    let smask_stream =
                        lopdf::Stream::new(smask_dict, alpha_bytes).with_compression(true);
                    ...
                    text_ops.push(Operation {
                        operator: "Do".into(),
                        operands: vec![lopdf::Object::Name(name.clone())],
                    });
                    text_ops.push(Operation {
                        operator: "Q".into(),
                        operands: vec![],
                    ...
                    if let Ok(text_bytes) = text_content.encode() {
                        ...
                        log::info!(
                            "lopdf hybrid: page {} text overlay added ({}x{} at x={:.1} y={:.1})",
                            page_idx,
                            w,
                            h,
                            x_pt,
                        ...
                        log::warn!(
                            "lopdf hybrid: page {} text overlay content encode failed",
                            page_idx
                        );
                    }
                }
                Err(e) => {
                    log::warn!(
                        "lopdf hybrid: page {} text overlay PNG decode failed: {}",
                        page_idx,
                        e
                    );
                }
            ...
            log::warn!(
                "lopdf hybrid: page {} render_text_overlay returned None",
                page_idx
            );
        }
            ...
            for slot_idx in filled_slot_indices.iter() {
                ...
                match image::load_from_memory(&png_bytes) {
                    Ok(rgba_img) => {
                        ...
                        let img_stream =
                            lopdf::Stream::new(img_dict, rgb_bytes).with_compression(true);
                        ...
                        num_ops.push(Operation {
                            operator: "Do".into(),
                            operands: vec![lopdf::Object::Name(name.clone().into_bytes())],
                        });
                        num_ops.push(Operation {
                            operator: "Q".into(),
                            operands: vec![],
                        ...
                        let num_content = lopdf::content::Content {
                            operations: num_ops,
                        };
                        if let Ok(num_bytes) = num_content.encode() {
                            ...
                            log::info!(
                                "lopdf hybrid: page {} number added at x={:.1} y={:.1}",
                                page_idx,
                                x_pt,
                                y_pt
                            );
                    ...
                    Err(e) => {
                        log::warn!(
                            "lopdf hybrid: page {} number decode failed: {}",
                            page_idx,
                            e
                        );
                    }
        ...
        if request.settings.watermark {
            ...
            if !wm_text.is_empty() {
                ...
                if let Some((png_bytes, _w, _h)) = render_watermark(
                    &text_font, &wm_text, &wm_color, wm_opacity, wm_size, wm_angle,
                ) {
                    match image::load_from_memory(&png_bytes) {
                        Ok(rgba_img) => {
                            let rgba_img = rgba_img.to_rgba8();
                            ...
                            for slot_idx in filled_slot_indices.iter() {
                                ...
                                if request.settings.reimburse_mode {
                                        ...
                                        .position(|(idx, _, _, _)| *idx == slot_idx)
                                    {
                                        ...
                                        let slot_h_pt = slot.h_mm * MM_TO_PT;
                                        let (mut scx, mut scy) =
                                            match request.settings.fit_mode.as_str() {
                                                "fill" => (slot_w_pt / vis_w, slot_h_pt / vis_h),
                                                "original" => (1.0, 1.0),
                                                "custom" => {
                                                    let c = (slot_w_pt / vis_w)
                                                        .min(slot_h_pt / vis_h)
                                                        * request.settings.custom_scale;
                                                    (c, c)
                                                }
                                                _ => {
                                                    let c =
                                                        (slot_w_pt / vis_w).min(slot_h_pt / vis_h);
                                                    (c, c)
                                                }
                                            };
                                        if let Some(a) = adj {
                                        ...
                                        if let Some(a) = adj {
                                            if a.offset_x != 0.0 {
                                                ox += a.offset_x * MM_TO_PT;
                                            }
                                            if a.offset_y != 0.0 {
                                                oy -= a.offset_y * MM_TO_PT;
                                            }
                                        }
                        ...
                        Err(e) => {
                            log::warn!(
                                "lopdf hybrid: page {} watermark decode failed: {}",
                                page_idx,
                                e
                            );
                        }
        ...
        let cutline_content = if request.settings.reimburse_mode {
            build_reimburse_cutline_ops_lopdf(
                pw_pt,
                ph_pt,
                request.settings.reimburse_height.unwrap_or(120.0),
            )
        } else if request.settings.cutline {
            // Calculate footer cut line position
            let has_footer = request.settings.page_num
                || request.settings.print_date
                || request
                    .settings
                    .footer_text
            ...
            let footer_cutline_y_pt = if has_footer {
                if request.settings.custom_fm && request.settings.footer_margin > 0.0 {
                    ...
                    let line_count = (if request.settings.page_num || request.settings.print_date {
                        1
                    } else {
                        0
                    }) + (if request
        ...
        if request.settings.border {
            if let Some(border_ops) = build_border_ops_lopdf(
                &slot_positions,
                &slot_adjustments,
                &page_form_xobjs,
                &request.settings,
        ...
        let content_id = output_doc.add_object(
            lopdf::Stream::new(lopdf::Dictionary::new(), content_bytes).with_compression(true),
        );
        ...
        resources_dict.set(
            b"XObject".to_vec(),
            lopdf::Object::Dictionary(xobjects_dict),
        );
    ...
    std::fs::write(output_path, &pdf_buf).map_err(|e| format!("写入文件失败: {}", e))?;
    if let Some(ref cb) = &on_progress {
        cb("save", 1, 1);
    }
...
fn build_reimburse_cutline_ops_lopdf(
    ...
    ops.push(Operation {
        operator: "w".into(),
        operands: vec![lopdf::Object::Real(0.5)],
    });
    ops.push(Operation {
        operator: "d".into(),
        operands: vec![
            lopdf::Object::Array(vec![lopdf::Object::Integer(3), lopdf::Object::Integer(3)]),
            lopdf::Object::Integer(0),
        ],
    });
    ops.push(Operation {
        operator: "G".into(),
        operands: vec![lopdf::Object::Real(0.0)],
    });
    ...
    for k in 1..seg_count {
        ...
        ops.push(Operation {
            operator: "m".into(),
            operands: vec![lopdf::Object::Real(0.0), lopdf::Object::Real(y)],
        });
        ops.push(Operation {
            operator: "l".into(),
            operands: vec![lopdf::Object::Real(page_w_pt), lopdf::Object::Real(y)],
        });
        ops.push(Operation {
            operator: "S".into(),
            operands: vec![],
...
fn build_cutline_ops_lopdf(
    ...
    if cols > 1 {
        ...
        for c in 1..cols {
            ...
            ops.push(Operation {
                operator: "m".into(),
                operands: vec![lopdf::Object::Real(x), lopdf::Object::Real(y_start)],
            });
            ops.push(Operation {
                operator: "l".into(),
                operands: vec![lopdf::Object::Real(x), lopdf::Object::Real(y_end)],
            });
            ops.push(Operation {
                operator: "S".into(),
                operands: vec![],
...
fn build_border_ops_lopdf(
    ...
    ops.push(Operation {
        operator: "w".into(),
        operands: vec![lopdf::Object::Real(1.0)],
    });

    ops.push(Operation {
        operator: "d".into(),
        operands: vec![
            lopdf::Object::Array(vec![lopdf::Object::Integer(1), lopdf::Object::Integer(0)]),
            lopdf::Object::Integer(0),
        ],
    });
    ...
    for (adj_idx, (layout_slot_idx, _xobj_id, src_w_pt, src_h_pt)) in form_xobjs.iter().enumerate()
    {
        ...
        ops.push(Operation {
            operator: "m".into(),
            operands: vec![lopdf::Object::Real(x1), lopdf::Object::Real(y1)],
        });
        ops.push(Operation {
            operator: "l".into(),
            operands: vec![lopdf::Object::Real(x2), lopdf::Object::Real(y1)],
        });
        ops.push(Operation {
            operator: "l".into(),
            operands: vec![lopdf::Object::Real(x2), lopdf::Object::Real(y2)],
        });
        ops.push(Operation {
            operator: "l".into(),
            operands: vec![lopdf::Object::Real(x1), lopdf::Object::Real(y2)],
        });
        ops.push(Operation {
            operator: "h".into(),
            operands: vec![],
