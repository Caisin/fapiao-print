...
- **独立发票提取库**：新增跨平台 `invoice-extractor` Rust crate，统一解析 PDF、OFD、XML 和 OCR 结果
- **递归目录导入**：一次选择发票目录，递归扫描子目录中所有支持的发票文件
- **发票管理界面**：支持按时间、主体、金额、票种筛选和常用财务维度汇总
- **重复与错误复核**：重复发票号特殊标记，识别异常可视化，详情弹窗可预览票面、查看关联文件并复制文件目录
- **完整结构化字段**：新增税率、中文大写金额、开票人、商品明细，并区分增值税普通发票与专用发票

### 🐛 修复

- 兼容非标准 ToUnicode CMap、Form XObject 字段和 WPS 可编辑 PDF 文字层
- 修复 PDF 坐标镜像导致的购销方颠倒、CJK 拆字、个体工商户名称截断与复杂明细金额错配
- 修复多页 PDF 按页误计为多个文件，并优化 PDF 可视内容边界以减少排版空白

### 📦 构建与平台

- macOS 使用 Core Graphics 渲染 PDF，本地开发与 OCR 解析不依赖 Windows 组件
- tag 发布同时生成 Windows 四个产物与 macOS Universal 轻量/OCR 两个 DMG
